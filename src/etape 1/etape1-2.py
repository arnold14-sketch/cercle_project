...
def demander_entier (message : str,minimum : int | None=None,maximum : int | None=None) -> int | None:
        ...
        if nbre_cercle>0:
            if type(nbre_cercle) is not float:
                for i in range(nbre_cercle):
                    nom_cercle=input("Renseignez le nom du cercle "+str(i+1)+" :")
...
def ajout_membre(cercles : dict[str,dict[str,Any]],minimum_membre:int |None=None ,maximum_membre :int | None=None):
            ...
            if nbre_membre>minimum_membre or nbre_membre<maximum_membre:
                if type(nbre_membre) is int:
                    for j in range(nbre_membre):
                        cercle=input('Dans quel cercle souhaitez-vous rajouter le membre?')
                        if cercle in cercles:

                            membre=input("Nom du membre à ajouter")

                            liste_total_membres=[element for liste in cercles.values() for element in liste]
        ...
        if nbre_suppression>0:
            if type(nbre_suppression) is int:
                ...
                for j in range(nbre_suppression):
                    ...
                    liste_totale_membres=[element for liste in dict.values() for element in liste]
